...
function(__file_append_line out_file line)
...
function(__harvest_dir_core directory out_file wix_path_var relative_to depth components_id)
    ...
    foreach(__path ${dir_glob})
        ...
        if (IS_DIRECTORY ${__path})
            ...
            __file_append_line(${out_file} "${__dir_indentation}<Directory Id=\"${__dir_id}\" Name=\"${__windows_like_path}\">")
            __harvest_dir_core(${__path} ${out_file} ${wix_path_var} ${relative_to} ${__next_depth} __components_id_subdir)
            __file_append_line(${out_file} "${__dir_indentation}</Directory>")
            ...
            __file_append_line(${out_file} "${__dir_indentation}<Component Id=\"${__component_id}\" Guid=\"{${__component_raw_guid}}\">")
            __file_append_line(${out_file} "${__component_indentation}<File Id=\"${__file_id}\" KeyPath=\"yes\" Source=\"\$(${wix_path_var})\\${__windows_like_path}\"/>")
            __file_append_line(${out_file} "${__dir_indentation}</Component>")
        endif()
    endforeach()
...
function(harvest_directory directory out_file wix_includes wix_path_var directory_ref_id component_group)
    ...
    __file_append_line(${__target_file} "<?xml version=\"1.0\" encoding=\"UTF-8\" ?>")
    __file_append_line(${__target_file} "<Wix xmlns=\"http://wixtoolset.org/schemas/v4/wxs\">")

    __file_append_line(${__target_file} "")
    foreach(inc ${wix_includes})
        __file_append_line(${__target_file} "${includes_indentation}<?include ${inc} ?>")
    endforeach()
    __file_append_line(${__target_file} "")

    __file_append_line(${__target_file} "${fragments_indentation}<Fragment>")
    __file_append_line(${__target_file} "${dir_ref_id_indentation}<DirectoryRef Id=\"${directory_ref_id}\">")
    __harvest_dir_core("${directory}" ${__target_file} "${wix_path_var}" "${__given_dir_abspath}" "2" components_id)
    __file_append_line(${__target_file} "${dir_ref_id_indentation}</DirectoryRef>")
    __file_append_line(${__target_file} "${fragments_indentation}</Fragment>")

    __file_append_line(${__target_file} "")
    __file_append_line(${__target_file} "${fragments_indentation}<Fragment>")
    __file_append_line(${__target_file} "${cg_indentation}<ComponentGroup Id=\"${component_group}\">")
    foreach(__id ${components_id})
        __file_append_line(${__target_file} "${components_indentation}<ComponentRef Id=\"${__id}\"/>")
    endforeach()
    __file_append_line(${__target_file} "${cg_indentation}</ComponentGroup>")
    __file_append_line(${__target_file} "${fragments_indentation}</Fragment>")
    __file_append_line(${__target_file} "")

    __file_append_line(${__target_file} "</Wix>")
